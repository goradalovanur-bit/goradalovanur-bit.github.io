...
(function () {
  ...
  var MAX_ASSET_BYTES = 90 * 1024 * 1024; // 90MB — safely under GitHub's ~100MB git blob ceiling
  ...
  function cacheEls() {
    ...
    els.fieldsPicker = document.getElementById('fieldsPicker');
    ...
    els.commitProgress = document.getElementById('commitProgress');
    els.commitProgressFill = document.getElementById('commitProgressFill');
    els.commitProgressLabel = document.getElementById('commitProgressLabel');
  ...
  /* ---------------------------------------------------------------------
     Git Data API — used instead of the Contents API for the actual
     commit. The Contents API means one PUT (and one commit) per file,
     sequentially: for several images and videos staged at once, that's
     slow, litters history with a commit per asset, and — since every
     staged file's full base64 string sits in memory simultaneously
     until commit — the biggest source of the "memory crash with
     multiple large files" complaint. Building one tree from blobs
     created concurrently, then a single commit, fixes both: fewer
     round-trips, one clean commit, and blobs are freed as each
     completes rather than all held until a final serial pass.
     ------------------------------------------------------------------- */
  ...
  /* A hung request (dropped connection, GitHub-side stall) would otherwise
     leave the commit spinner running forever with no error and no way
     out short of reloading the page. AbortController gives every Git
     Data API call a hard ceiling — generous enough for a big blob
     upload on a slow connection, short enough that a genuine hang
     surfaces as a clear, recoverable error instead of an infinite spin. */
  async function fetchWithTimeout(url, options, timeoutMs) {
    ...
    try {
      ...
      if (e.name === 'AbortError') throw new Error('Request timed out — GitHub did not respond in time. Check your connection and try again.');
      throw e;
    } finally {
      clearTimeout(timer);
    }
  ...
  async function ghJson(url, options, timeoutMs) {
  ...
  async function ghGetRef() {
    var s = state.settings;
    return ghJson(ghRepoUrl() + '/git/ref/heads/' + encodeURIComponent(s.branch), { headers: ghHeaders() });
  }

  async function ghGetCommit(sha) {
    return ghJson(ghRepoUrl() + '/git/commits/' + sha, { headers: ghHeaders() });
  }

  async function ghCreateBlob(base64Content) {
    return ghJson(ghRepoUrl() + '/git/blobs', {
      method: 'POST', headers: Object.assign({ 'Content-Type': 'application/json' }, ghHeaders()),
      body: JSON.stringify({ content: base64Content, encoding: 'base64' })
    }, 120000); // large asset uploads get a longer ceiling than metadata calls
  }

  async function ghCreateTree(baseTreeSha, entries) {
    return ghJson(ghRepoUrl() + '/git/trees', {
      method: 'POST', headers: Object.assign({ 'Content-Type': 'application/json' }, ghHeaders()),
  ...
  async function ghCreateCommit(message, treeSha, parentSha) {
    return ghJson(ghRepoUrl() + '/git/commits', {
      method: 'POST', headers: Object.assign({ 'Content-Type': 'application/json' }, ghHeaders()),
  ...
  async function ghUpdateRef(commitSha) {
    var s = state.settings;
    return ghJson(ghRepoUrl() + '/git/refs/heads/' + encodeURIComponent(s.branch), {
      method: 'PATCH', headers: Object.assign({ 'Content-Type': 'application/json' }, ghHeaders()),
      body: JSON.stringify({ sha: commitSha })
    });
  }

  /* Runs async jobs with at most `limit` in flight at once — enough
     concurrency to be fast without tripping GitHub's abuse-rate limits
     the way an unbounded Promise.all over a dozen files could. */
  async function runWithConcurrency(items, limit, worker) {
    ...
    var lanes = [];
    for (var i = 0; i < Math.min(limit, items.length); i++) lanes.push(runOne());
    await Promise.all(lanes);
    return results;
  }

  function showCommitProgress(done, total, label) {
    if (!els.commitProgress) return;
    els.commitProgress.hidden = false;
    var pct = total ? Math.round((done / total) * 100) : 0;
    els.commitProgressFill.style.width = pct + '%';
    els.commitProgressLabel.textContent = label || (done + ' / ' + total + ' files');
  }
  function hideCommitProgress() {
    if (!els.commitProgress) return;
    els.commitProgress.hidden = true;
    els.commitProgressFill.style.width = '0%';
  }
  ...
  function handleMediaTokenFile(file) {
    ...
    if (!checkFileSize(file, file.name)) return;
    if (isVideo) {
  ...
  function openEditor(id, sourceType) {
    ...
    if (id && sourceType) {
      ...
      els.fQuestion.value = ''; els.fSolution.value = '';
      ...
      els.fPdfUrl.value = ''; els.fPdfThumb.value = '';
      els.fCourseDesc.value = ''; els.fCourseCoverUrl.value = '';
  ...
  var AUTOSAVE_FIELD_IDS = [
    'f-question', 'f-solution', 'f-content', 'f-image',
    'f-language', 'f-code-snippet', 'f-animation-url', 'f-github-url',
    'f-video-url', 'f-yt-desc', 'f-pdf-url', 'f-pdf-thumb',
    'f-course-desc', 'f-course-cover-url'
  ];
  ...
  function saveItemFromForm() {
    ...
    if (type === 'problem') {
      ...
      record.course = deriveFieldLabel();
  ...
  /* Derives the human-readable "Field of Mathematics" label (used for the
     course/field badge, the category filter facet, and search) directly
     from whichever Field chips are currently selected in state.tags —
     the single multi-select picker is now the only source of this,
     replacing the old free-text input. Joins multiple selections with a
     comma; empty when no field is selected, matching prior behavior for
     items with no course/field set. */
  function deriveFieldLabel() {
    var fields = getFields();
    var selected = state.tags
      .map(function (t) { return String(t).toLowerCase(); })
      .map(function (slug) { var f = fields.find(function (f) { return f.slug === slug; }); return f ? f.label : null; })
      .filter(Boolean);
  ...
  function renderTagChips() {
    ...
    var fieldSlugs = getFields().map(function (f) { return f.slug; });
    state.tags.forEach(function (tag) {
      if (fieldSlugs.indexOf(String(tag).toLowerCase()) !== -1) return; // shown via the Fields picker instead
  ...
  /* Multi-select Field of Mathematics picker — a set of toggle chips
     built from getFields(), reading/writing directly into state.tags
     (the same array the public AND-filter and card badges already use)
     rather than a separate schema field. Clicking a chip adds/removes
     that field's slug; an item can carry any number at once. */
  function renderFieldsPicker() {
    if (!els.fieldsPicker) return;
    var fields = getFields();
    var activeSlugs = state.tags.map(function (t) { return String(t).toLowerCase(); });
    els.fieldsPicker.innerHTML = '';
    if (!fields.length) {
      els.fieldsPicker.innerHTML = '<p class="hint" style="margin:0;">No fields defined yet — add some in the Fields tab.</p>';
      return;
    }
    fields.forEach(function (f) {
      var chip = document.createElement('button');
      chip.type = 'button';
      chip.className = 'field-chip' + (activeSlugs.indexOf(f.slug) !== -1 ? ' is-active' : '');
      chip.textContent = f.label;
      chip.addEventListener('click', function () {
        var idx = state.tags.findIndex(function (t) { return String(t).toLowerCase() === f.slug; });
        if (idx !== -1) state.tags.splice(idx, 1);
        else state.tags.push(f.slug);
        renderTagChips();
        state.dirty = true;
      });
      els.fieldsPicker.appendChild(chip);
    });
  }
  ...
  function handlePdfFile(file) {
    ...
    reader.onerror = function () { toast('error', 'Could not read file', file.name + ' could not be read.'); };
    reader.readAsDataURL(file);
  }
  ...
  /* Size guardrail — checked before FileReader ever touches the file, so
     an oversized drop fails fast with a clear message instead of the
     browser hanging or crashing while trying to hold a huge base64
     string in memory, and instead of a commit later failing against
     GitHub's own blob size ceiling. */
  function formatBytes(n) {
    if (n >= 1024 * 1024) return (n / (1024 * 1024)).toFixed(1) + ' MB';
    return Math.round(n / 1024) + ' KB';
  }
  function checkFileSize(file, contextLabel) {
    if (file.size <= MAX_ASSET_BYTES) return true;
    var isVideo = file.type.indexOf('video/') === 0;
    var advice = isVideo
      ? ' Videos this large are also a poor fit for a git repository — hosting it on YouTube and using the YouTube URL field (or an "Animation URL" link) will load faster for visitors and has no size limit at all.'
      : ' Try a smaller source file — images are already compressed automatically, so this limit is almost never hit for photos.';
    toast('error', 'File too large', contextLabel + ' is ' + formatBytes(file.size) + ', over the ' + formatBytes(MAX_ASSET_BYTES) + ' limit.' + advice);
    return false;
  }
    ...
    Array.prototype.forEach.call(fileList, function (file) {
      ...
      if (isVideo) {
        ...
        reader.onload = function () {
          ...
          var fileName = genId() + '-' + sanitizeFileName(file.name);
          state.pendingUploads.push({ folder: folder, fileName: fileName, base64: base64, bytes: file.size });
          state.media.push({ id: genId(), kind: 'video', url: folder + fileName, placement: 'full', width: 100, caption: '', _previewDataUrl: reader.result });
          state.dirty = true;
          renderMediaManagerList();
        };
        reader.onerror = function () { toast('error', 'Could not read file', file.name + ' — try a smaller file or a different format.'); };
        reader.readAsDataURL(file);
        return;
      }
  ...
  async function commitToGitHub() {
    ...
    // Gather every pending asset into one flat list so it can all be
    // blobbed concurrently and land in a single commit alongside data.json.
    var files = [];
    if (state.pendingPdf) files.push({ path: normalizeFolder(state.settings.assetPath) + state.pendingPdf.fileName, base64: state.pendingPdf.base64 });
    if (state.pendingPdfThumb) files.push({ path: 'assets/img/' + state.pendingPdfThumb.fileName, base64: state.pendingPdfThumb.base64 });
    if (state.pendingAvatar) files.push({ path: 'assets/img/' + state.pendingAvatar.fileName, base64: state.pendingAvatar.base64 });
    state.pendingUploads.forEach(function (u) { files.push({ path: u.folder + u.fileName, base64: u.base64 }); });

    // Defense in depth: re-check size from the staged base64 even though
    // every upload path already rejected oversized files before staging.
    var oversized = files.filter(function (f) { return (f.base64.length * 0.75) > MAX_ASSET_BYTES; });
    if (oversized.length) {
      toast('error', 'File too large', oversized.map(function (f) { return f.path.split('/').pop(); }).join(', ') + ' exceeds the ' + formatBytes(MAX_ASSET_BYTES) + ' limit and was not committed. Remove it from Additional Media and try a smaller file.');
      return;
    }
    ...
    var totalSteps = files.length + 1;
    showCommitProgress(0, totalSteps, files.length ? ('Uploading 0 / ' + files.length + ' asset files…') : 'Preparing commit…');

    try {
      var doneCount = 0;
      var blobEntries = await runWithConcurrency(files, 3, async function (f) {
        var blob = await ghCreateBlob(f.base64);
        doneCount++;
        showCommitProgress(doneCount, totalSteps, 'Uploading ' + doneCount + ' / ' + files.length + ' asset files…');
        return { path: f.path, mode: '100644', type: 'blob', sha: blob.sha };
      });

      var payload = buildPayload();
      var dataBlob = await ghCreateBlob(utf8ToBase64(JSON.stringify(payload, null, 2)));
      blobEntries.push({ path: state.settings.dataPath, mode: '100644', type: 'blob', sha: dataBlob.sha });
      showCommitProgress(totalSteps, totalSteps, 'Finalizing commit…');

      var refData = await ghGetRef();
      var baseCommitSha = refData.object.sha;
      var baseCommit = await ghGetCommit(baseCommitSha);
      var newTree = await ghCreateTree(baseCommit.tree.sha, blobEntries);
      var commitMessage = 'Update content: ' + totalItemCount() + ' items' +
        (files.length ? ', ' + files.length + ' asset' + (files.length === 1 ? '' : 's') : '');
      var newCommit = await ghCreateCommit(commitMessage, newTree.sha, baseCommitSha);
      await ghUpdateRef(newCommit.sha);

      state.dataSha = dataBlob.sha;
      state.pendingPdf = null;
      state.pendingPdfThumb = null;
      var hadAvatar = !!state.pendingAvatar;
      state.pendingAvatar = null;
      state.pendingUploads = [];
      state.dirty = false;
      updateCommitBar();
      if (hadAvatar) updateAvatarPreview();
      toast('success', 'Committed', (files.length ? files.length + ' asset file' + (files.length === 1 ? '' : 's') + ' and ' : '') + 'data.json updated on ' + state.settings.branch + ' in one commit. GitHub Pages will redeploy shortly.');
    } catch (err) {
