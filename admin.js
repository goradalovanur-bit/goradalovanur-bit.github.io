...
(function () {
  ...
  function renderItems() {
    ...
    list.forEach(function (item) {
      ...
      row.innerHTML =
        ...
        '<div><span class="status-pill is-' + status + '">' + (status === 'draft' ? 'Draft' : 'Published') + '</span></div>' +
  ...
  function formatRichText(raw) {
    ...
    var escaped = escapeHtmlText(extracted.text);
  ...
  function runSplitEditorCmd(cmd, ta) {
    if (cmd === 'bold') return insertAtCursor(ta, '**', '**', 'qalın mətn');
    if (cmd === 'heading') return insertAtCursor(ta, '\n### ', '\n', 'Bölmə başlığı');
    if (cmd === 'math-inline') return insertAtCursor(ta, '$', '$', 'x^2');
    if (cmd === 'math-block') return insertAtCursor(ta, '\n$$\n', '\n$$\n', 'x = \\frac{-b \\pm \\sqrt{b^2-4ac}}{2a}');
  ...
  function collectAllMediaUrls() {
    ...
    (state.raw.code_animations || []).forEach(function (c) { addFromList(c.media); });
  ...
  function restoreAutosaveDraft() {
    var draft = readAutosaveDraft();
    if (!draft) return;
    openEditor(draft.id || null, draft.id ? draft.sourceType : null);
