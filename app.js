...
(function () {
  ...
  function cacheEls() {
    ...
    els.viewModeToggle = document.getElementById('viewModeToggle');
  ...
  /* Grid (the existing rich card layout — thumbnails, previews, badges,
     unchanged) vs. Compact (dense scannable rows: title, one-line
     description, type badge, date) view switch for the feed. Choice is
     remembered across visits via localStorage, best-effort. */
  function bindViewModeToggle() {
    if (!els.viewModeToggle) return;
    var saved;
    try { saved = localStorage.getItem('academicPortalViewMode'); } catch (e) { /* ignore */ }
    if (saved === 'compact' || saved === 'grid') state.viewMode = saved;
    applyViewMode();
    els.viewModeToggle.querySelectorAll('[data-mode]').forEach(function (btn) {
      btn.addEventListener('click', function () {
        state.viewMode = btn.dataset.mode;
        try { localStorage.setItem('academicPortalViewMode', state.viewMode); } catch (e) { /* ignore */ }
        applyViewMode();
      });
    });
  }
  ...
  function applyViewMode() {
    ...
    if (els.feedContainer) els.feedContainer.classList.toggle('is-compact', state.viewMode === 'compact');
  }
  ...
  function itemFacets(item) {
    var facets = [];
    // A comma means this is a composite of multiple Field-of-Mathematics
    // labels (from the multi-select picker) rather than one distinct
    // category value — each of those fields is already its own facet via
    // the tags loop below, so faceting the whole joined string too would
    // just create a one-off, unmatchable filter pill per combination.
    if (item.category && item.category.indexOf(',') === -1) facets.push('cat:' + item.category);
