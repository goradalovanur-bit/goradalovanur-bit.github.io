...
(function () {
  ...
  /* Pre-defined "Fields of Mathematics" tags (slug -> display label).
     Used to build multi-select filter facets and to render tag chips
     with proper Title Case / acronym capitalization across the app. */
  var FIELD_TAGS = {
    'algebra': 'Algebra',
  ...
  function render() {
    ...
    var filtered = sectionItems.filter(function (item) {
      if (state.activeFilters.length) {
        ...
        var matches = state.activeFilters.some(function (f) { return facets.indexOf(f) !== -1; });
  ...
  var KICKER_LABEL = { problem: 'Solved Problem', story: 'Essay & Story', code: 'Code & Animation', youtube: 'YouTube', pdf: 'PDF' };
